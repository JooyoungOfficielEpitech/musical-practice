#!/usr/bin/env python3
"""
Hypothesis gate attempt 3: fix notehead classification to preserve ambiguous notes.

Key fix from attempt 2:
- Noteheads without clear stem direction should be AMBIGUOUS (kept in both images)
- Only components with STRONG stem evidence get erased from one image
- This preserves chords that will be split post-hoc by voice_splitter
"""
...
class HypGate3:
    """Erase-from-original with ambiguity preservation."""
    ...
    def classify_noteheads(self, stems: np.ndarray) -> List[str]:
        """
        Classify each notehead as up/down/ambiguous via stem direction.
        Ambiguous = no strong evidence for either direction.
        """
        classifications = []
        stem_threshold = 3
        ...
        for notehead in self.noteheads:
            ...
            direction = "ambiguous"
            if stem_ru > stem_threshold and stem_ru > stem_ld * 2:
                direction = "up"
            elif stem_ld > stem_threshold and stem_ld > stem_ru * 2:
                direction = "down"
        ...
        log.info(
            f"Classified noteheads: {n_up} up, {n_down} down, {n_ambig} ambiguous"
        )
    ...
    def classify_components(
        ...
        Ambiguous components stay in both images (mask is 0 for both).
        """
        num_features, labeled = cv2.connectedComponents(binary_no_staff)
        ...
        for comp_id in range(1, num_features + 1):
            ...
            if direction == "up":
                up_mask[comp_mask] = 1
                erased_bboxes.append(
                    (comp_y.min(), comp_x.min(), comp_y.max(), comp_x.max())
                )
            elif direction == "down":
    ...
    def build_erase_images(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Ambiguous components stay in both.
        """
    ...
    def run_homr(self, img: np.ndarray, voice_name: str, variant: str = "base") -> Optional[Dict]:
        ...
        img_file = f"{self.output_dir}/hyp3_{self.crop_name}_{voice_name}_{variant}.png"
    ...
    def test_dilation_variant(self, up_img: np.ndarray, down_img: np.ndarray) -> Optional[Dict]:
        """Test with dilation of white-fill mask."""
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
        up_dilated = cv2.dilate(up_img, kernel, iterations=1)
        down_dilated = cv2.dilate(down_img, kernel, iterations=1)

        self.visual_iterations += 1
        log.info(f"Visual iteration {self.visual_iterations}: testing dilation variant")

        up_result = self.run_homr(up_dilated, "up", "dilate2")
        down_result = self.run_homr(down_dilated, "down", "dilate2")
        ...
        if (
            ...
            log.info("SUCCESS: Dilation variant passed!")
            return {
                "variant": "dilate2",
                "up": up_result,
                "down": down_result,
...
if __name__ == "__main__":
    ...
    for crop_path, crop_name in crops:
        ...
        tester = HypGate3(crop_path, crop_name, output_dir)
        result = tester.run()
