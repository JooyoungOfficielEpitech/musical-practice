#!/usr/bin/env python3
"""
Hypothesis gate attempt 4: physical notehead reconstruction per voice.

Key insight: erasing stems doesn't change notehead y-positions.
Homr determines pitch purely from vertical position, so noteheads in same place
yield same pitches regardless of stem direction shown.

Solution: For each voice, build an image with ONLY that voice's noteheads,
stems, and beams at their original positions. Reconstruct the visual by
copying pixels from original where classified components belong to that voice.

This approach accepts that noteheads at original positions = same pitch,
and relies on post-processing to assign pitches to voices correctly.
Tests whether homr even RUNS on reconstructed-but-incomplete images.
"""
...
class HypGate4:
    """Reconstruct per-voice images by selective pixel copying."""
    ...
    def classify_components(
        self, binary_no_staff: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
        """Classify all components; return up_mask, down_mask."""
        num_features, labeled = cv2.connectedComponents(binary_no_staff)
    ...
    def build_reconstructed_images(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Build images by selective pixel copying from original.
        up_img: original pixels where components are classified up + staff lines
        down_img: original pixels where components are classified down + staff lines
        """
        ...
        up_mask, down_mask = self.classify_components(binary_no_staff)

        h, w = self.gray.shape
        up_img = np.ones((h, w), dtype=np.uint8) * 255
        down_img = np.ones((h, w), dtype=np.uint8) * 255

        up_img[up_mask > 0] = self.gray[up_mask > 0]
        down_img[down_mask > 0] = self.gray[down_mask > 0]

        for top, bot in self.staff_lines:
            up_img[top:bot+1, :] = self.gray[top:bot+1, :]
            down_img[top:bot+1, :] = self.gray[top:bot+1, :]

        log.info("Built reconstructed images")
        return up_img, down_img

    def run_homr(self, img: np.ndarray, voice_name: str, variant: str = "base") -> Optional[Dict]:
        img_file = f"{self.output_dir}/hyp4_{self.crop_name}_{voice_name}_{variant}.png"
        ...
        try:
            ...
            if result.returncode != 0:
                log.warning(f"homr failed: {result.stderr[:200]}")
                return None
    ...
    def test_base(self) -> Optional[Dict]:
        up_img, down_img = self.build_reconstructed_images()
        if up_img is None or down_img is None:
            return None

        self.visual_iterations += 1
        log.info(f"Visual iteration {self.visual_iterations}: reconstructed images")
        ...
        if (
            ...
            log.info("SUCCESS: Reconstruction variant passed!")
            return {
        ...
        log.warning(f"Variant failed: up MIDI {up_result['mean_midi']}, down MIDI {down_result['mean_midi']}")
        return None
...
if __name__ == "__main__":
    ...
    for crop_path, crop_name in crops:
        ...
        tester = HypGate4(crop_path, crop_name, output_dir)
        result = tester.run()
