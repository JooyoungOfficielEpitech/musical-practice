...
def process_single_staff(
    ...
    for voice_img, voice_name, voice_idx in [
        ...
        if voice_xml is None:
            ...
            # On the success path, detect x-noteheads separately for each voice image.
            # The up_img and down_img differ from the merged image, so x-positions
            # must be re-detected per voice. This strategy (a) ensures accurate marking
            # for each voice while avoiding false positives from the other voice.
            if x_positions:
                voice_processed, voice_x_pos, _ = replace_x_noteheads(voice_img)
                if voice_x_pos:
                    if X_MARKING_ENABLED:
                        voice_xml = mark_x_noteheads_in_xml(voice_xml, voice_x_pos)
                    log.info(f"{tag} {voice_name}: Marked {len(voice_x_pos)} X-noteheads from separated image")
