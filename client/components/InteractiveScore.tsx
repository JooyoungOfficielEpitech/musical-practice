...
type WebViewIncoming =
  | { type: "ready" }
  | { type: "notePress"; noteIndex: number; timeMs?: number | null }
...
<body>
  ...
  <script>
    ...
    function seekToMs(ms){if(!osmd||!osmd.cursor||timeTable.length===0)return;var lo=0,hi=timeTable.length-1,step=0;while(lo<=hi){var mid=(lo+hi)>>1;if(timeTable[mid]<=ms){step=mid;lo=mid+1;}else{hi=mid-1;}}if(step===currentStep)return;stepCursor(step);}
...
export const InteractiveScore = memo(function InteractiveScore({
      ...
      try {
        ...
        switch (data.type) {
          case "ready":
            ...
            break;
          case "notePress":
            onNotePress?.(data.noteIndex, data.timeMs ?? null);
            break;
          case "error":
  ...
  // Reload the score when the XML itself changes after the initial load (note
  // edits) — the WebView stays mounted, OSMD just re-renders the new content.
  const prevXmlRef = useRef(musicXml);
  useEffect(() => {
